...
Reporter模块通过AOP探针或者硬编码的方式嵌入到业务代码中，负责Span的生成和上报。
Transport模块是Trace数据上报通道，支持HTTP、Kafka、Scribe等方式。
Colletor模块负责Trace数据的接收，并将Trace数据写入到中央存储中。
Storage部分为存储组件，默认是In-Memory存储，并支持Cassandra、Elasticsearch、MySQL等存储系统。
