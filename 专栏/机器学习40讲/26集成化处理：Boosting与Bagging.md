...
\[ w_i \\leftarrow w_i \\cdot \\exp \[\\alpha_m I(x_i)\]\]对分类错误的样本\(I(x_i) = 1\)，分类正确的样本\(I(x_i) = -1\)，这样做的作用就是放大误分类样本的权重。新计算出的权重经过归一化处理后，就可以用来继续训练下一个弱分类器，直到集成出强分类器为止。强分类器的数学表达式为
\[ F(x) = {\\rm sign} \[\\sum\\limits_{m = 1}^M \\alpha_m f_m(x)\] \]如果将二分类任务进行推广，那么上面表达式中的符号函数sign\((\\cdot)\)就可以去掉，得到的就是一个进行了基函数扩展的线性回归模型。
