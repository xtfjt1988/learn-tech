...
在一套创建过 Redis 集群的环境，未作彻底清理的情况下，使用 redis-trib 工具再次创建集群；
...
借助 redis-cli 登录各个节点，执行 cluster flushall 和 cluster reset 命令；
...
登录本地客户端（redis-cli）查看集群状态信息（cluster info），显示只有两个主节点在线；
